...
`ContextData` 常用字段：`requestId` `method` `path` `ip` `host` `adminId` `username` `tenantId` `isSuper` `perms` `dataScope` `dataScopeDeptIds`。
...
框架在 `App.start()` 中扫描 `modules`、注册 IoC、挂路由。宿主中间件在 `src/middleware/`：
...
| 项 | 说明 |
|----|------|
| `default.ts` + `dev.ts` / `prod.ts` | 深度合并；**禁止 `.env`** |
| `system.port` | HTTP 端口 |
...
|------|------|
| 同 `hook` | 通常只能启用一个 |
| 配置 | 只读 `pluginInfo.config`，插件内禁止 `.env` |
