...
| API | 用法 |
|-----|------|
| `registerHost({ … })` | **必填**：静态绑定 Db/Cache/Queue/Auth/Task/Plugin/Module/Socket/middleware/`scan`；`index.ts` 须先 `import './host'` |
...
import './host' // registerHost 静态绑定（build 与 IoC 同一模块图）
...
| 文件 | 作用 |
|------|------|
| `src/host.ts` | `registerHost({ Db…Socket, middleware, scan })` |
| `src/host-scan.ts` | **生成物**（勿手改）：`bun run gen:host-scan` / 各 build 脚本前置 |
