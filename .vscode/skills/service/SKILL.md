...
自动导入：模板列不含 `id`/`createTime`/`updateTime`/`deleteTime`/`tenantId`；清洗后整批 `svc.add`（开 softDelete → 批量 upsert：先查再分类写；`upsert:insert` 则批量插入，撞实体 unique 略过）；`importUniqueKeys` 仅 softDelete→upsert 时覆盖表 unique。
